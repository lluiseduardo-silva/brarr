...
- `brarr-cli` (Phase 5 + remote subcommand): binary `brarr` with two subcommands. `brarr search` fans out locally from a TOML tracker config. `brarr remote --addr host:port --token <t> --tmdb <id>` dispatches the search to a running `brarr-orchestrator` over gRPC (no local tracker config needed). Both render results through the same text/JSON formatter.
...
216 tests pass (`cargo test --workspace --all-targets`). `INITIAL_PROMPT.md` remains the authoritative spec — consult before adding crates, dependencies, or making architectural decisions.
