...
- `brarr-orchestrator` (pull-path performance, `src/search.rs` + `src/ttl_cache.rs` + `src/provider_cache.rs`): the Torznab/Newznab feed handlers call `run_search_cached` instead of `run_search`. Sonarr/Radarr blow one Interactive Search up into many near-identical indexer requests (per season/episode, once per configured feed — and the `/torznab` + `/newznab` feeds run the *same* full fan-out, differing only in which protocol's decisions they render), so without dedup each one re-fans-out to every provider and re-persists a `searches`+`decisions` set, stalling the *arr UI for minutes. Three guards: (1) a short-TTL `TtlCache<SearchKeys, SearchRunOutcome>` in `AppState` (`SEARCH_CACHE_TTL`, 60s; `ttl=0` disables) collapses the duplicate burst onto a single fan-out — the admin UI and poller still call `run_search` directly for fresh data; (2) `fan_out` caps each provider's dispatch with `tokio::time::timeout` (`PER_PROVIDER_BUDGET`, 15s) so one slow/timing-out tracker can't hold the whole search for the underlying client's 30s×retry window — it's reported as a timeout failure and the rest proceed; (3) a `ProviderClientCache` keyed by provider id + a config fingerprint reuses the built UNIT3D/Newznab HTTP client (warm `reqwest` connection pool + TLS) across requests; plugins are still rebuilt per call.

545 tests pass (`cargo test --workspace --all-targets`). `INITIAL_PROMPT.md` remains the authoritative spec — consult before adding crates, dependencies, or making architectural decisions.
