...
- `brarr-tracker-newznab` (Newznab/Torznab): async `reqwest` client for Newznab indexers (NZBGeek, DrunkenSlug, NZB.su, etc.) and Torznab Usenet-shape trackers. Hand-rolled `quick-xml` parser that collects `<newznab:attr>` entries (including repeated `audio`/`subs`) into a `HashMap` and maps `language`/`audio`/`subs` to `ReleaseEnrichment`. Implements `TrackerProvider` via `search_by_imdb` (movie-search axis).
- `brarr-cli` (Phase 5 + remote subcommand): binary `brarr` with two subcommands. `brarr search` fans out locally from a TOML tracker config. `brarr remote --addr host:port --token <t> {--tmdb <id> | --imdb <ttN>}` dispatches the search to a running `brarr-orchestrator` over gRPC (no local tracker config needed). Both render results through the same text/JSON formatter.
...
- `brarr-orchestrator` (Phase 6b + cross-crate integration): tonic gRPC server (`Brarr` service: `Search`, `ListTrackers`, `RecentSearches`) + Axum admin web UI (dashboard, trackers CRUD, releases history, search detail) backed by SQLite via `sqlx`. Server-side rendered with Askama templates + HTMX, Tailwind via CDN, no frontend build pipeline. Search fan-out goes through the `brarr_core::TrackerProvider` trait so UNIT3D direct clients, Newznab indexers, and WASM plugins share one pipeline. Trackers table has a `kind` column (`unit3d` / `newznab`) and a nullable `plugin_path` — when set the row is served by `WasmTrackerProvider`; otherwise the `kind` picks between Unit3D and Newznab clients. The gRPC `Search` RPC accepts `tmdb_id` and/or `imdb_id`; orchestrator dispatch prefers IMDb for Newznab rows and TMDb everywhere else, falling back to the other axis when missing. A single `wasmtime::Engine` lives in `AppState`. Binary `brarr-orchestrator` launches both servers concurrently.
...
- `brarr-orchestrator` (Torznab indexer endpoint, `src/web/torznab.rs`): exposes `/torznab/api` so Sonarr/Radarr can use brarr as a single virtual Newznab/Torznab indexer that fans out across every configured tracker. Dispatch on `?t=`: `caps` returns an XML capability advert (movie-search: yes; tv-search: no); `movie&{tmdbid|imdbid}` runs the full search pipeline and renders the kept decisions as an RSS feed with `torznab:attr` blocks (category, size, seeders, peers, leechers, grabs, tmdb, imdb); `search`/`tvsearch` return a valid empty feed (free-text and TVDB+season axes aren't built yet); unknown `t=` values return `400` with a Newznab `<error code="202">` payload. Auth accepts either `?apikey=<BRARR_AUTH_TOKEN>` (the *arr-native way) or `Authorization: Bearer <token>`. Categories are inferred from `ReleaseKind` + `Resolution`: movies vs TV via `S##E##` regex heuristic on the title, then `2030/5030` SD, `2040/5040` HD, `2045/5045` UHD, plus `2050` BluRay as a secondary subcat for movie BluRay rips. Per-item download URL falls back to a synthetic `brarr:///download/<release_id>` placeholder when the persisted decision row doesn't carry an upstream URL; an explicit `/torznab/download/{tracker}/{release}` proxy route remains future work. Hand-rolled XML via `quick-xml`'s element builder — no Askama templates touch this surface.

251 tests pass (`cargo test --workspace --all-targets`). `INITIAL_PROMPT.md` remains the authoritative spec — consult before adding crates, dependencies, or making architectural decisions.
